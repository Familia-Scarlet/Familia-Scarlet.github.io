...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div>`,101),g={start:"2"},x=i(`<div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;p&gt;性别：{{gender}}&lt;/p&gt;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="属性绑定指令" tabindex="-1"><a class="header-anchor" href="#属性绑定指令" aria-hidden="true">#</a> 属性绑定指令</h3><p>==v-bind==：为DOM元素的属性动态绑定值，由于会经常使用到属性绑定指令,故规定可用==:缩写==。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;input type=&quot;text&quot; v-bind:placeholder=&quot;tips&quot;&gt;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="双向绑定指令" tabindex="-1"><a class="header-anchor" href="#双向绑定指令" aria-hidden="true">#</a> 双向绑定指令</h3><p>可用来辅助开发者在不操作DOM的情况下，快速获取表单数据。</p><p>可以将v-model与v-bind对比学习，</p><p>v-bind只能将Model渲染到View，</p><p>而v-model==既能将Model渲染到View，也能将View同步到Model==。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;p&gt;用户的名字是：{{username}}&lt;/p&gt;
&lt;input type=&quot;text&quot; v-model=&quot;username&quot;&gt;
&lt;input type=&quot;text&quot; v-bind:value=&quot;username&quot;&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>v-model可用于如下标签，只要有value属性即可使用</p><ul><li>input的radio、checkbox、text</li><li>textArea</li><li>select</li></ul><h3 id="v-model指令的修饰符" tabindex="-1"><a class="header-anchor" href="#v-model指令的修饰符" aria-hidden="true">#</a> v-model指令的修饰符</h3><ul><li>.number：自动将用户输入内容转换成数值类型。</li><li>.trim：自动将用户输入内容去掉首尾空白字符。</li><li>.lazy：懒更新，只在被绑定对象失去焦点时才进行数据的改变。</li></ul><h3 id="条件渲染指令" tabindex="-1"><a class="header-anchor" href="#条件渲染指令" aria-hidden="true">#</a> 条件渲染指令</h3><p>辅助开发者按需控制DOM节点的显示与隐藏，有两个v-if和v-show。</p><p>v-show通过style=&quot;display:none;&quot;方式隐藏DOM对象，性能更好。</p><p>v-if通过修改DOM树来隐藏DOM对象，需要动态修改DOM树，性能差些。</p><p>v-if可用于不需要频繁隐藏显示的情况。实际开发中，不需要考虑性能问题，直接使用v-if即可，因为短，且能和v-else-if，v-else配套使用。</p><p>==PS：vue指令的属性是js代码。==</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;p v-if=&quot;flag&quot;&gt;这是被v-if控制&lt;/p&gt;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果需要使用索引，vue指令用v-for=&quot;(item,index) in list&quot;</p><p>==官方建议，只要使用了v-for指令就一定要绑定一个：key属性，并将id作为key的值，key的值只能是字符串或数字类型。==</p><p>key的值必须具有唯一性，如果重复会报错。</p><p>index不具备唯一性，所以不能作为key的值，因为index与列表相关，列表变长可能会导致index发生变化。index值当key没有任何意义。</p><h3 id="input标签与label标签混用时的一个问题" tabindex="-1"><a class="header-anchor" href="#input标签与label标签混用时的一个问题" aria-hidden="true">#</a> input标签与label标签混用时的一个问题</h3><p>关键在于要将input的id与label的for属性绑定起来，这样就能将这两一一联系。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;input type=&quot;checkbox&quot; :id=&quot;&#39;cb&#39;+item.id&quot; v-model=&quot;item.status&quot;&gt;
&lt;label :for=&quot;&#39;cb&#39;+item.id&quot; v-if=&quot;item.status&quot;&gt;已启用&lt;/label&gt;
&lt;label :for=&quot;&#39;cb&#39;+item.id&quot; v-else&gt;已禁用&lt;/label&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="过滤器" tabindex="-1"><a class="header-anchor" href="#过滤器" aria-hidden="true">#</a> 过滤器</h3><p>过滤器用于==文本的格式化==，可用于插值表达式和v-bind属性绑定。</p><p>过滤器应该被添加在Javascript表达式尾部，由管道符&quot;|&quot;进行调用。</p><p>过滤器需要在filters中进行定义，定义时要写形参，且必须有返回值。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>//&quot;|capitalize&quot;就是过滤器的用法，这里的作用是首字母大写，本质是个function函数
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>==为了编写方便，优先考虑方法格式的侦听器。如果有需求才使用对象格式侦听器==</p><h4 id="深度侦听" tabindex="-1"><a class="header-anchor" href="#深度侦听" aria-hidden="true">#</a> 深度侦听</h4><p>如果侦听的对象是对象类型，而不是数值或字符串类型，那么对象中的属性发生变化时并不会触发侦听器。解决方式是使用对象格式的侦听器，使用==deep==进行对象的深度侦听。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>watch:{
	info:{
		handler(newVal){
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="对象子属性侦听" tabindex="-1"><a class="header-anchor" href="#对象子属性侦听" aria-hidden="true">#</a> 对象子属性侦听</h4><p>但是原始的深度侦听有缺陷，如果不是想侦听的属性发生变化，会产生多余的处理。于是就有了子属性的侦听。给对象的子属性加上==单引号==即可。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&#39;info.username&#39;(newVal){
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="vue组件" tabindex="-1"><a class="header-anchor" href="#vue组件" aria-hidden="true">#</a> Vue组件</h2><p>vue是支持组件化的框架。</p><p>组件的后缀名是.vue，之前接触的App.vue文件的本质就是一个vue组件。</p><p>每个.vue组件都由3部分组成</p><ul><li><p>template：组件的结构</p><p>vue组件中只允许有一个根元素，即template标签下只允许有一个标签，而不允许有同级的其他标签</p></li><li><p>script：组件的js行为、数据等</p><p>script标签中有个固定写法，默认导出。</p><p>.vue组件中的data不能像之前一样指定对象，组件中的data必须是个函数。</p><p>于是，数据定义到data函数的return中即可。</p><p>==但是==，只是data是这样，但methods的用法依旧是使用对象的格式。</p><p>此时，这里的this是组件的实例，而不再是vue实例。</p><p>最后一定要进行默认导出。</p></li></ul><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;script&gt;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li></ul><h3 id="vue组件的导入" tabindex="-1"><a class="header-anchor" href="#vue组件的导入" aria-hidden="true">#</a> Vue组件的导入</h3><p>组件在被封装好时候是没有什么关系的，当被使用的时候，根据彼此的嵌套关系，形成了==父子关系==或==兄弟关系==</p><p>使用组件的三个步骤：</p><ol><li>在<code>&lt;script&gt;</code>标签下，使用import语法导入需要使用到的组件，如在根组件App.vue中导入Left.vue</li></ol><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>import Left from &#39;@/components/Left.vue&#39;
...
&lt;/style&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>主要是在使用第三方组件库且有修改样式的需求的时候，才会常用&quot;/deep/&quot;</p><p>==原理==：&quot;/deep/&quot;本质就是属性，且是父组件的属性，通过属性选择器先选择到父组件，嵌套使用标签选择器，这样就可以对子组件的样式进行修改了。</p><h3 id="vue组件实例对象" tabindex="-1"><a class="header-anchor" href="#vue组件实例对象" aria-hidden="true">#</a> vue组件实例对象</h3><p>vue组件是通过vue-template-compiler渲染到网页上的，本身无法被浏览器解析。</p><p>当使用到vue组件的标签的时候，就相当于创建了一个组件的实例。</p><h2 id="生命周期" tabindex="-1"><a class="header-anchor" href="#生命周期" aria-hidden="true">#</a> 生命周期</h2><p>创建---运行----销毁</p><h3 id="生命周期函数" tabindex="-1"><a class="header-anchor" href="#生命周期函数" aria-hidden="true">#</a> 生命周期函数</h3><p>new Vue()-(初始化事件和生命周期函数)-beforeCreate-(初始化props、data、methods)-created</p><p>beforeMount-mounted</p><p>beforeUpdate-updated</p><p>beforeDestroy-destroyed</p><h3 id="创建" tabindex="-1"><a class="header-anchor" href="#创建" aria-hidden="true">#</a> 创建</h3><p><img src="`+s+'" alt="截图_20230225181437"></p><h4 id="beforecreate" tabindex="-1"><a class="header-anchor" href="#beforecreate" aria-hidden="true">#</a> beforeCreate</h4><p>在这阶段还无法使用props、data、methods，一般没什么用</p><h4 id="created" tabindex="-1"><a class="header-anchor" href="#created" aria-hidden="true">#</a> created</h4><p>==非常重要的一个==，在这阶段已经可以使用methods方法了，常用来进行ajax请求来取数据，并将取到的数据转存到data中。但此时的DOM元素还没有被渲染上页面，所以无法对DOM树进行操作。</p><h3 id="编译页面结构" tabindex="-1"><a class="header-anchor" href="#编译页面结构" aria-hidden="true">#</a> 编译页面结构</h3><p><img src="'+l+'" alt="截图_20230225183226"></p><p>有vue-template-compiler基于vue指令和模板在内存中编译html结构，但还没有构建DOM结构。</p><h3 id="渲染-由html结构到dom树-与更新" tabindex="-1"><a class="header-anchor" href="#渲染-由html结构到dom树-与更新" aria-hidden="true">#</a> 渲染(由HTML结构到DOM树)与更新</h3><p><img src="'+r+'" alt="截图_20230225183921"></p><h4 id="beforemounte" tabindex="-1"><a class="header-anchor" href="#beforemounte" aria-hidden="true">#</a> beforeMounte</h4><p>和beforeCreate一样较少使用，因为这个时候还无法操作DOM树，html页面结构还没有被渲染到浏览器中。</p><h4 id="mounted" tabindex="-1"><a class="header-anchor" href="#mounted" aria-hidden="true">#</a> mounted</h4><p>==常使用==，因为这个时候已经可以对DOM结构进行操作了</p><h4 id="beforeupdate" tabindex="-1"><a class="header-anchor" href="#beforeupdate" aria-hidden="true">#</a> beforeUpdate</h4><p>此时的DOM结构依旧是旧的，还没发生变化，而==data中的数据==已经是==最新==的值了。</p><h4 id="updated" tabindex="-1"><a class="header-anchor" href="#updated" aria-hidden="true">#</a> updated</h4><p>==常用==，此时的DOM结构与data中的数据都是新的了。</p><h3 id="销毁" tabindex="-1"><a class="header-anchor" href="#销毁" aria-hidden="true">#</a> 销毁</h3><p>==极少使用==，通过v-show、v-if来摧毁</p><p><img src="'+u+`" alt="截图_20230225184536"></p><h2 id="组件间传值" tabindex="-1"><a class="header-anchor" href="#组件间传值" aria-hidden="true">#</a> 组件间传值</h2><h3 id="父向子传值" tabindex="-1"><a class="header-anchor" href="#父向子传值" aria-hidden="true">#</a> 父向子传值</h3><p>父可以用子标签的方式使用子组件，于是可以采用自定义属性的方式进行传值。父向子传简单类型的值的方式是复制一份再传过去，如果是像对象那样的复杂类型，则是传过去引用。</p><h3 id="子向父传值" tabindex="-1"><a class="header-anchor" href="#子向父传值" aria-hidden="true">#</a> 子向父传值</h3><p>子组件向父组件传值需要使用==自定义事件==，也可以使用ref由父组件直接控制，下面介绍的是==自定义事件==的方法。</p><p>子组件触发按钮，从而执行add()方法，add()方法会emit出numchange信号，由子组件的监听器接收到后，触发numchange事件，进而执行在父组件中的getNewCount方法。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>//子组件
...
&lt;SideBar&gt;
	...
	&lt;template&gt;
&lt;/SideBar&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="路由" tabindex="-1"><a class="header-anchor" href="#路由" aria-hidden="true">#</a> 路由</h2><p>路由就是指对应关系，指的是==哈希地址(url地址)==和==页面(组件)==之间的关系。</p><p>#，锚链接，不会导致页面的刷新，只会导致页面的变化，也会影响浏览历史记录。哈希地址其实也就是锚链接。</p><p>从#开始往后的地址都是哈希地址，也是锚链接。</p><h3 id="前端路由的工作方式" tabindex="-1"><a class="header-anchor" href="#前端路由的工作方式" aria-hidden="true">#</a> 前端路由的工作方式</h3><ol><li>用户点击了前端上的路由链接</li><li>导致了URL地址栏中hash值发生了变化</li><li>前端路由监听到了Hash地址的变化</li><li>前端路由会把当前Hash地址对应的组件渲染到浏览器中</li></ol><p><img src="`+o+`" alt="截图_20230301114550"></p><h3 id="前端路由原理" tabindex="-1"><a class="header-anchor" href="#前端路由原理" aria-hidden="true">#</a> 前端路由原理</h3><p>使用window.onhashchange()来监听哈希地址的变化，但不够完善，自己封装路由是非常困难的，别自己写路由，一般用三方库实现路由。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>created(){
	window.onhashchange() = ()=&gt;{
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol start="3"><li>在main.js中导入路由模块，目的是拿到路由实例。在模块导入的时候，如果给定的是文件夹，则==默认导入==该文件夹下命名为 index.js 的文件。</li></ol><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>import router from &#39;@/router/index.js&#39;

new Vue({
	render: h=&gt;h(App),
	router: router
}).$mount(&#39;#app&#39;)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="路由的基本用法" tabindex="-1"><a class="header-anchor" href="#路由的基本用法" aria-hidden="true">#</a> 路由的基本用法</h3><ol><li>用<code>&lt;a&gt;</code>标签建立哈希地址</li><li>用<code>&lt;router-view&gt;</code>标签建立占位符，组件会在这里进行展示</li><li>配置路由模块的哈希地址与组件的对应关系，这里的对应关系可称为==路由规则==</li></ol><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>import Vue from &#39;vue&#39;
import VueRouter from &#39;vue-router&#39;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="嵌套路由" tabindex="-1"><a class="header-anchor" href="#嵌套路由" aria-hidden="true">#</a> 嵌套路由</h3><p>通过路由实现组件的嵌套展示，嵌套如下。</p><p><img src="`+m+`" alt="截图_20230301160248"></p><p>在父路由规则中配合属性children，且子路由的哈希链接地址==不要加&#39;/&#39;==，同样也需要导入子组件。同时子路由重定向，要在父路由的规则中配置，也可以使用默认子路由。</p><p>当子路由规则中，某条规则的path为&#39;&#39;时候，会在父组件中默认展示出对应路由对应的子组件。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>// 配置嵌套路由的规则
const router = new VueRouter({
	routes:[
		{path:&#39;/&#39;,redirect:&#39;/home&#39;}
		{path:&#39;/home&#39;,component:Home},
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="导航守卫" tabindex="-1"><a class="header-anchor" href="#导航守卫" aria-hidden="true">#</a> 导航守卫</h2><p>导航守卫可以控制路由的访问权限，示意图如下</p><p><img src="`+p+`" alt="截图_20230301191556"></p><h3 id="全局前置守卫" tabindex="-1"><a class="header-anchor" href="#全局前置守卫" aria-hidden="true">#</a> 全局前置守卫</h3><p>每次发生路由的导航跳转时，都会触发全局前置守卫，使用如下：</p><p>在路由模块index.js中，使用vuerouter的实例调用beforeEach，形参是个回调函数，每次发生路由变化时候都会被调用一次，回调函数的三个参数是to、from、next。</p><p>to是将要访问的路由的信息对象</p><p>from是将要离开的路由的信息对象</p><p>next是一个函数，</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>const router = new VueRouter({...})
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="项目结构的梳理" tabindex="-1"><a class="header-anchor" href="#项目结构的梳理" aria-hidden="true">#</a> 项目结构的梳理</h2><p>component和view都是放组件的，但view目录是放需要通过路由切换加载出来的组件。</p><h2 id="遗漏的课程内容" tabindex="-1"><a class="header-anchor" href="#遗漏的课程内容" aria-hidden="true">#</a> 遗漏的课程内容</h2><p>第8天--P204~P226</p><p>第5天--P127~P144</p>`,386);function f(q,w){return a(),d("div",null,[h,e("ol",g,[e("li",null,"=="+t()+"==：插值表达式，只能用在内容，不能用在属性，数据驱动视图，一般是占位符，不会覆盖默认内容",1)]),x])}const j=n(b,[["render",f],["__file","Vue.html.vue"]]);export{j as default};
